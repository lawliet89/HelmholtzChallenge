...
std::string str_cat(char *path, char *ext){
    return std::string(path) + std::string(ext);
}
 ...
 *
 */
double * read_coords_2D(const char * filename, int * nodes){
  double * res;
  int len, size, a, i;
  FILE *f;
  f = fopen(filename, "r");
...
int * read_cell_node_map_2D(const char * filename, int * cells, int * cell_size){
  int * res;
  int len, size, a, i;
  FILE *f;
  f = fopen(filename, "r");
